...
{
	public enum MovementMode { walk, sprint };
	...
	public MovementMode currentGait = MovementMode.walk;
	...
	void Update () 
	{
		if (GameManager.isPaused == false)
        {
			...
			{
				currentGait = MovementMode.sprint;
			}
			else
			{
				currentGait = MovementMode.walk;
			}
			...
			if (GameManager.isPaused == false)
			{
				...
				if (currentGait == MovementMode.walk)
				{
					movePos = new Vector3(newDir.x * walkSpeed, movePos.y, newDir.z * walkSpeed);
				}
				else
				{
					movePos = new Vector3(newDir.x * sprintSpeed, movePos.y, newDir.z * sprintSpeed);
				}
